...
- Selection operations persist their mask in sprite metadata. The synchronized build stores row runs to preserve ellipse/compound masks between calls, and can read legacy rectangular bounds. Metadata is preserved when selecting/deselecting; selections created by an older bounds-only build cannot regain their lost mask.
